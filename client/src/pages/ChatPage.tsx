        ...
        return (
          ...
          >
            <div
              ...
              style={{
                background: own ? 'var(--color-own-bubble)' : 'var(--color-other-bubble)',
                color: own ? 'var(--color-own-bubble-text)' : 'var(--color-other-bubble-text)',
...
export function ChatPage() {
  ...
  const fetchRoom = async () => {
    try {
      const { data } = await axios.get<Room>(`/api/rooms/${id}`);
      setRoom(data);
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Error al cargar room', 'error');
    }
  };

  const fetchMessages = async () => {
    try {
      const { data } = await axios.get<Message[]>(`/api/rooms/${id}/messages`);
      setMessages(data);
    } catch (err) {
  ...
  const handleSend = async (e: Event) => {
    ...
    setSending(true);
    try {
      const { data } = await axios.post<Message>(`/api/rooms/${id}/messages`, {
  ...
  const isOwnMessage = (message: Message) => message.created_by.email === auth?.user.email;
      ...
      >
        <form onSubmit={handleSend} class="max-w-3xl mx-auto flex gap-2">
          ...
          <button
            ...
            style={{
              background: sending ? 'var(--color-text-muted)' : 'var(--color-primary)',
              color: 'white',
              border: 'none',
            }}
          >
