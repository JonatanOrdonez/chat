...
export function RoomsPage() {
  ...
  const fetchRooms = async () => {
    try {
      ...
      showToast(err instanceof Error ? err.message : 'Error al cargar rooms', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateRoom = async (e: Event) => {
    ...
    setCreating(true);
    try {
      const { data } = await axios.post<Room>('/api/rooms', { name: newRoomName.trim() });
      setRooms((prev) => [data, ...prev]);
      setNewRoomName('');
      showToast('Room creado', 'success');
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Error al crear room', 'error');
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleDeleteRoom = async (e: Event, roomId: string) => {
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : 'Error al eliminar room', 'error');
    } finally {
      setDeleting(null);
    }
  };
                  ...
                  {isOwner(room) && (
                    <button
                      ...
                      style={{
                        background: 'transparent',
                        border: '1px solid var(--color-border)',
                        color: deleting === room.id ? 'var(--color-text-muted)' : 'var(--color-danger)',
                      }}
                    >
